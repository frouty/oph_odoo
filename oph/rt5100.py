# -*- coding: utf-8 -*-
"""
utilities to parse the RT5100 log file
And convert datas from RT5100 to ODOO datas.
"""
import re, os
import logging
...
def map2odoofields(raw):
    ...
    @return: an other closer to the ODOO model.
    res:{'UCVA': [['va_bin_extended', '1.25'], ['va_bin', '1.25'], ['va_ol_extended', '0.63'], ['va_ol', '0.63'], ['va_or_extended', '0.1'], ['va_or', '0.1']], 'Rx': [['va_bin_extended', '0.32'], ['va_bin', '0.32'], ['va_ol_extended', '0.32'], ['va_ol', '0.32'], ['va_or_extended', '0.25'], ['va_or', '0.25'], ['add_os', '+4.50'], ['add_od', '+3.75'], ['sca_os', '+16.0', '-4.75', '130'], ['sca_or', '+11.75', '-3.5', '175']], 'BCVA': [['va_bin_extended', '1.6'], ['va_bin', '1.6'], ['va_ol_extended', '2.0'], ['va_ol', '2.0'], ['va_or_extended', '0.32'], ['va_or', '0.32'], ['add_os', '+3.50'], ['add_od', '+3.50'], ['sca_os', '-1.25', '-5.25', '130'], ['sca_or', '+5.25', '-8.75', '175']], 'AR': [['va_bin_extended', '<0.04'], ['va_bin', '<0.04'], ['va_ol_extended', '0.8'], ['va_ol', '0.8'], ['va_or_extended', '0.4'], ['va_or', '0.4'], ['sca_os', '-0.5', '-6.75', '25'], ['sca_or', '+6.0', '-6.25', '175']]}
    
    each item must be process again before inserting in the measurement dictionnary with makeSCAdict method
    finalDict=finalDict.update(dict[makeSCAdict(item)])
    """
...
def getandformat_values(rxlist = [regexSCA, regexADD, regexVA], log_path = os.path.expanduser('~') + '/rt5100rs232/tmp.log'):
    ...
    map2odoofields method will do that for you.
    """
