...
#     """
#      Inherits partner and adds request information in the partner form 
#     """
...
class mail_compose_message(osv.Model):
    ...
    def send_mail(self, cr, uid, ids, context = None):
        context = context or {}
        if context.get('default_model') == 'oph.iol.order' and context.get('default_res_id') and context.get('mark_order_as_sent'):
            context = dict(context, mail_post_autofollow = True)
            self.pool.get('oph.iol.order').write(cr, uid, [context['default_res_id']], {'sent': True}, context = context)
            self.pool.get('oph.iol.order').write(cr, uid, [context['default_res_id']], {'state': 'open'}, context = context)
